...
function App(){
  ...
  useEffect(()=>{request<{csrf:string}>('/auth/me').then(x=>{setCsrf(x.csrf);setEntry('login')}).catch(()=>{
    request<{needs_setup:boolean,web_setup_available:boolean}>('/auth/setup-status')
      .then(x=>setEntry(x.needs_setup?(x.web_setup_available?'setup':'unavailable'):'login'))
  ...
  async function doSetup(e:React.FormEvent){e.preventDefault();setError('');if(password.length<12){setError('密码至少需要 12 个字符。');return}if(password!==confirmation){setError('两次输入的密码不一致。');return}try{const x=await request<{csrf:string}>('/auth/setup',{method:'POST',body:JSON.stringify({password,confirmation})});setCsrf(x.csrf);setEntry('login');setPassword('');setConfirmation('')}catch(e){setError('创建失败：'+String(e))}}
  async function cancelSetup(){try{await request('/auth/cancel-setup',{method:'POST'});setStopped(true)}catch(e){setError('关闭失败：'+String(e))}}
  async function closeLocal(){try{await request('/auth/close-local',{method:'POST'});setStopped(true)}catch(e){setError('关闭失败：'+String(e))}}
  ...
  async function changeLocalPolicy(sourceId:string,contentPolicy:string){try{await request(`/v1/local/sources/${sourceId}/policy`,{method:'POST',body:JSON.stringify({content_policy:contentPolicy})},csrf);void refresh()}catch(e){setError(String(e))}}
  async function logout(){try{await request('/auth/logout',{method:'POST'},csrf)}finally{setCsrf('');setSessions([]);setDetail(null);setSelected(null)}}
  async function shutdown(){if(!window.confirm('关闭工作台？关闭后可从开始菜单重新打开。'))return;try{await request('/v1/local/shutdown',{method:'POST'},csrf);setStopped(true)}catch(e){setError('关闭失败：'+String(e))}}
  ...
  return <div className="shell"><aside className="sidebar"><div className="logo"><span className="brandIcon">✳</span><span>Agent<br/>Workbench</span></div><nav aria-label="主导航"><button className={view==='overview'?'active':''} onClick={()=>setView('overview')}>▦　总览</button><button className={view==='sessions'?'active':''} onClick={()=>setView('sessions')}>☷　会话</button><button className={view==='devices'?'active':''} onClick={()=>setView('devices')}>◉　设备与设置</button></nav><div className="sidebarFoot"><span className="liveDot"/> 单用户工作台<br/><small>仅展示已采集证据</small><button className="logout" onClick={()=>void logout()}>退出登录</button><button className="logout" onClick={()=>void shutdown()}>关闭工作台</button></div></aside>
    ...
    {view==='devices'&&<div className="settingsGrid">
      <section className="panel"><h2>已配对设备</h2><p className="muted">本机启动时自动配对并采集。</p>
        {devices.length?devices.map(d=><div className="deviceRow" key={d.id}><span className="deviceIcon">⌘</span><div><b>{d.name}</b><small>{d.id}</small><small>{d.environment}</small><small>{d.last_heartbeat?'最近心跳 '+time(d.last_heartbeat):'等待首次采集'}</small></div><span className="badge">{d.pending_count} 待传</span></div>):<Empty text="尚无设备"/>}
      </section>
      <section className="panel"><h2>本机数据来源</h2><p className="muted">默认只保存统计。开启正文后，仅新产生的消息会保存脱敏后的内容；内容可能仍包含隐私，请自行决定。</p>
        {sources.length?sources.map(s=><div className="sourceRow" key={s.id}><b>{s.agent} · {s.profile}</b><small>{s.last_error?'错误：'+s.last_error:s.last_scan?'最近扫描 '+time(s.last_scan):'尚未扫描'}</small>{s.execution_surface==='local'&&<label className="policyLabel">后续消息采集<select aria-label={`${s.agent} 正文采集策略`} value={JSON.parse(s.capability_json||'{}').content_policy||'stats_only'} onChange={e=>void changeLocalPolicy(s.id,e.target.value)}><option value="stats_only">仅统计</option><option value="full_content">保存脱敏后的正文</option></select></label>}</div>):<Empty text="当前没有发现 Codex 或 Hermes 来源"/>}
      </section>
      <section className="panel advancedPanel"><details><summary>添加另一台电脑</summary><p className="muted">先生成配对码，再在另一台电脑运行采集器。确认来源 ID 后登记。</p><button onClick={makePair}>生成 10 分钟配对码</button>{pairCode&&<div className="pairCode" aria-label="配对码">{pairCode}</div>}
        <form className="register" onSubmit={register}><label>来源 ID<input value={registration.id} onChange={e=>setRegistration({...registration,id:e.target.value})} required/></label><label>设备 ID<select value={registration.device_id} onChange={e=>setRegistration({...registration,device_id:e.target.value})} required><option value="">选择设备</option>{devices.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select></label><label>Agent<select value={registration.agent} onChange={e=>setRegistration({...registration,agent:e.target.value})}><option value="codex">Codex</option><option value="hermes">Hermes</option></select></label><label>Profile<input value={registration.profile} onChange={e=>setRegistration({...registration,profile:e.target.value})}/></label><label>采集策略<select value={registration.content_policy} onChange={e=>setRegistration({...registration,content_policy:e.target.value})}><option value="stats_only">仅统计</option><option value="full_content">正文脱敏后采集</option><option value="excluded">排除</option></select></label><button type="submit">确认登记</button></form>
      </details></section>
    </div>}
    </main></div>
}
